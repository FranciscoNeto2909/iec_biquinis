...
export default function Cart({ handleCloseCart, handleSetMsg }) {
    ...
    const text = cart.map(item => (
        `•%20${item.name},%20cor:%20${item.color},%20tam:%20${item.size},%20qnt:${item.quant}`
    )).join("%0A");
                        ...
                        cart.length > 0 ?
                            <>
                                ...
                                <div className="cart_body_footer">
                                    ...
                                    <a href={`https://wa.me/5585996585581?text=Olá%20gostaria%20de%20fazer%20um%20pedido:%0A${text}%0A%0AEndereço:%20${address.name}%0ACupom:${cupom === validCupom ? cupom : "nenhum"}`} className="cart_body_button">Finalizar compra</a>
                                </div>
                            </> :
                            <>
